		...
		<div class="container">
			<div class="row">
				...
				<div class="mp_port_nav">
					<a href="{{action('IndexController@portfolio')}}" class="mp_port_all">@lang('main.all')</a>
					@foreach ($cat as $elements)					
					<a href="{{url('portfolio/category/'.$elements->slug)}}">{{$elements->{'title_' . App::getLocale()} }}</a>
					@endforeach
				</div>
				<div class="mp_port_page_content_items">
					<div class="mp_port_page_content_item">
						@foreach ($port as $element)
					...
					@if (isset($elements) && isset($element))
					<div class="mp_port_page_more">
						<button class="port-more" data-cat-id="{{$elements->id}}" data-id="{{$element->id}}">@lang('main.show')</button>
					</div>
					@endif
				</div>
			</div>
