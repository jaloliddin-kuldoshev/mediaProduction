...
<div class="box">
	<div class="box-header with-border">
		...
		<div class="box-body">
			<form role="form" method="post" action="{{ route('portfolio.update', ['id' => $news->id]) }}"  enctype="multipart/form-data">
				...
				<div class="box-body">
					<div class="form-group">
						...
						<select class="form-control" name="services_id" required>
							<option value="" selected="selected" disabled>Выберите Услугу</option>
							@foreach($ser  as $cat)
							<option value="{{ $cat->id }}" @if($cat->id == $news->services_id) selected @endif>{{ $cat->title_ru }}</option>
							@endforeach
						</select>
						@if($errors->has('services_id'))

						<small class="text-danger">{{ $errors->first('services_id') }}</small>
					...
					<div class="form-group">
						...
						<select class="form-control" name="clients_id" required>
							<option value="" selected="selected" disabled>Выберите Клиента</option>
							@foreach($cli  as $cat)
							<option value="{{ $cat->id }}" @if($cat->id == $news->clients_id) selected @endif>{{ $cat->title_ru }}</option>
							@endforeach
						</select>
						@if($errors->has('clients_id'))

						<small class="text-danger">{{ $errors->first('clients_id') }}</small>
					...
					<div class="form-group">
						...
						<select class="form-control" name="categories_id" required>
							<option value="" selected="selected" disabled>Выберите Клиента</option>
							@foreach($cats  as $cat)
							<option value="{{ $cat->id }}" @if($cat->id == $news->categories_id) selected @endif>{{ $cat->title_ru }}</option>
							@endforeach
						</select>
